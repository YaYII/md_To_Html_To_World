...
async function main() {
  ...
  try {
    ...
    if (!packageCheck.hasDependencies) {
      ...
      solutions.push('2. 运行 pnpm install 安装依赖 (推荐)');
    }
  ...
  console.log('\n🔧 快速修复命令:');
  console.log('  pnpm install                   # 安装依赖 (推荐)');
  console.log('  pnpm run check-deps            # 重新检查依赖');
  console.log('  pnpm run check-network         # 重新检查网络');
  console.log('  pnpm run check-all             # 综合诊断');
