        ...
        sections: [
          {
            ...
            children: generateToc ? [
              ...
              new docx.TableOfContents("目录", {
                ...
                stylesWithLevels: [
                  new docx.StyleLevel("Heading1", 1),
                  new docx.StyleLevel("Heading2", 2),
                  new docx.StyleLevel("Heading3", 3)
                ],
              }),
              // 添加分页符
              new Paragraph({
  ...
  async convertFile(inputFile, outputFile) {
    try {
      ...
      await Packer.toBuffer(doc).then(buffer => {
        return fs.outputFile(outputFile, buffer);
      });
      
      console.log(`Word文档已保存到: ${outputFile}`);
      return doc;
    } catch (error) {
