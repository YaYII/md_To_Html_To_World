...
class ConvertToWordCommand {
    ...
    async convert(config) {
        ...
        const result = await this.pythonRunner.runScript(scriptPath, args, {
    ...
    async prepareConfigFile(config) {
        const configData = {
            format: {
        ...
        await vscode.workspace.fs.writeFile(vscode.Uri.file(tempPath), Buffer.from(JSON.stringify(configData, null, 2)));
        return tempPath;
    }
    async cleanupConfigFile(configPath) {
        try {
