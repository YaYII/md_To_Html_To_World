...
class Converter {
  ...
  async convert_file(input_file, output_file, keep_html = false) {
    try {
      ...
      return {
        success: true,
        message: writeResult ? writeResult.message : '转换成功',
      ...
      // 提供更友好的错误消息
      let friendlyMessage = '转换失败';
      
      if (error.message.includes('EBUSY')) {
        const fileName = path.basename(output_file);
        friendlyMessage = `目标文件 "${fileName}" 正在被其他程序使用。请关闭该文件后重试，或选择其他输出位置。`;
      } else if (error.message.includes('EACCES') || error.message.includes('EPERM')) {
        friendlyMessage = `没有权限访问目标文件。请检查文件权限或选择其他输出位置。`;
      } else {
        friendlyMessage = `转换失败: ${error.message}`;
      }
