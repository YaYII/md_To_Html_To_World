  ...
  convertHtml(htmlContent, basePath = '') {
    try {
      ...
      if (generateToc && this.tocProcessor && this.tocProcessor.getHeadingCount() > 0) {
        ...
        const coverInfo = {
          title: docConfig.cover_title || this.config.title || '文档标题',
          version: docConfig.cover_version || '1.0',
          author: docConfig.cover_author || this.config.creator || '文档作者',
          date: docConfig.cover_date || new Date().toLocaleDateString('zh-CN'),
          description: docConfig.cover_description || ''
        };
        ...
        if (tocPlaceholderIndex >= 0) {
          ...
          if (generateCover) {
            // 生成完整的三页结构：封面 + 目录 + 正文
            const coverElements = this.tocProcessor.generateCoverPage(coverInfo);
            documentContent = [...coverElements, ...beforeToc, ...tocElements, ...afterToc];
            console.log(`📋 已生成完整文档结构：第1页封面 + 第2页目录 + 第3页开始正文，共${this.tocProcessor.getHeadingCount()}个标题`);
          } else {
            // 只生成目录 + 正文
