...
class AutoDependencyInstaller {
    ...
    private async detectPackageManager(): Promise<string> {
        ...
        for (const manager of packageManagers) {
            try {
                ...
                if (await fs.pathExists(lockFile)) {
                    this.outputChannel.appendLine(`找到 ${lockFiles[manager as keyof typeof lockFiles]}，使用 ${manager}`);
                    return manager;
                }
                ...
                await new Promise<void>((resolve) => {
                    exec(`${manager} --version`, (error: any, stdout: string) => {
                        if (!error) {
                            this.outputChannel.appendLine(`${manager} 可用: ${stdout.trim()}`);
                            resolve();
                        } else {
        ...
        this.outputChannel.appendLine('未找到可用的包管理器，使用默认的 npm');
        return 'npm';
    }
