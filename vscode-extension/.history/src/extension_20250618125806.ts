    ...
    private async checkNodeJsEnvironment(): Promise<boolean> {
        ...
        try {
            ...
            if (!nodeAvailable) {
                ...
                this.outputChannel.appendLine('\n🚨 Node.js 环境缺失！');
                this.outputChannel.appendLine('请按照以下步骤安装 Node.js：');
                this.outputChannel.appendLine('1. 访问 Node.js 官网：https://nodejs.org/');
                this.outputChannel.appendLine('2. 下载并安装 LTS 版本（推荐）');
                this.outputChannel.appendLine('3. 安装完成后重启 VS Code/Cursor');
                this.outputChannel.appendLine('4. 重新激活插件');
                
                // 显示一个更明显的错误提示
                vscode.window.showErrorMessage(
                    'Markdown to Word 插件需要 Node.js 环境。请安装 Node.js 后重启编辑器。',
                    '下载 Node.js'
                ).then(selection => {
