...
class AutoDependencyInstaller {
    ...
    private async autoInstallNodeJs(): Promise<boolean> {
        try {
            this.outputChannel.appendLine('📥 正在下载 Node.js...');
            ...
            return await vscode.window.withProgress({
                ...
                // 创建临时目录
                const tempDir = path.join(require('os').tmpdir(), 'nodejs-install');
                await fs.ensureDir(tempDir);
                
                // 下载文件
                const downloadSuccess = await this.downloadFile(downloadUrl, tempDir, progress);
                if (!downloadSuccess) {
                    return false;
                }
                
                progress.report({ increment: 50, message: "安装 Node.js..." });
                
                // 安装Node.js
                const installSuccess = await this.installNodeJs(tempDir, platform, progress);
            ...
            return new Promise((resolve) => {
                ...
                https.get(url, (response: any) => {
                    ...
                    response.on('data', (chunk: any) => {
                        ...
                        const percent = Math.round((downloadedSize / totalSize) * 30); // 30% for download
                        progress.report({ 
                            increment: 0, 
                            message: `下载中... ${Math.round((downloadedSize / totalSize) * 100)}%` 
                        });
                    });
    ...
    /**
     * @description 安装Node.js
     */
    private async installNodeJs(tempDir: string, platform: string, progress: vscode.Progress<{message?: string, increment?: number}>): Promise<boolean> {
        try {
            const { exec } = require('child_process');
            const path = require('path');
            const fs = require('fs');
            ...
            this.outputChannel.appendLine(`📦 准备安装: ${filePath}`);
            
            if (platform === 'win32') {
                return await this.installNodeJsWindows(filePath, progress);
            } else {
                return await this.installNodeJsUnix(filePath, platform, progress);
            }
            
        } catch (error) {
            this.outputChannel.appendLine(`❌ 安装过程出错: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }

    /**
     * @description Windows系统安装Node.js
     */
    private async installNodeJsWindows(filePath: string, progress: vscode.Progress<{message?: string, increment?: number}>): Promise<boolean> {
        try {
            const { exec } = require('child_process');
            const path = require('path');
            const fs = require('fs');
            ...
            // 解压ZIP文件
            progress.report({ message: "解压安装包..." });
            
            const extractDir = path.join(path.dirname(filePath), 'nodejs');
            ...
            return new Promise((resolve) => {
                exec(extractCommand, async (error: any, stdout: string, stderr: string) => {
                    if (error) {
                    ...
                    try {
                        ...
                        // 将Node.js复制到程序文件目录
                        const installDir = path.join(process.env.PROGRAMFILES || 'C:\\Program Files', 'nodejs');
                        
                        progress.report({ message: "复制文件到安装目录..." });
                        
                        // 复制文件
                        const copyCommand = `xcopy "${nodePath}" "${installDir}" /E /I /Y`;
                        
                        exec(copyCommand, (copyError: any) => {
                            if (copyError) {
                                this.outputChannel.appendLine(`❌ 复制文件失败: ${copyError.message}`);
                                resolve(false);
                                return;
                            }
                            
                            // 添加到系统PATH
                            progress.report({ message: "配置环境变量..." });
                            
                            this.addToWindowsPath(installDir).then((pathSuccess) => {
                                if (pathSuccess) {
                                    this.outputChannel.appendLine('✅ Node.js 安装完成');
                                    resolve(true);
                                } else {
                                    this.outputChannel.appendLine('⚠️ Node.js 安装完成，但环境变量配置可能需要手动设置');
                                    resolve(true); // 仍然认为安装成功
                                }
                            });
                        });
                        
                    } catch (err) {
                        this.outputChannel.appendLine(`❌ 安装过程出错: ${err}`);
                        resolve(false);
                    }
                });
            });
            ...
            this.outputChannel.appendLine(`❌ Windows安装失败: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }

    /**
     * @description Unix系统安装Node.js (macOS/Linux)
     */
    private async installNodeJsUnix(filePath: string, platform: string, progress: vscode.Progress<{message?: string, increment?: number}>): Promise<boolean> {
        try {
            const { exec } = require('child_process');
            const path = require('path');
            const os = require('os');
            ...
            const extractCommand = filePath.endsWith('.tar.gz') 
                ? `tar -xzf "${filePath}" -C "${path.dirname(filePath)}"` 
                : `tar -xJf "${filePath}" -C "${path.dirname(filePath)}"`;
            ...
            return new Promise((resolve) => {
                exec(extractCommand, async (error: any, stdout: string, stderr: string) => {
                    ...
                    try {
                        ...
                        const dirs = await require('fs').promises.readdir(parentDir);
                        const nodeDir = dirs.find((dir: string) => dir.startsWith('node-') && !dir.endsWith('.tar.gz') && !dir.endsWith('.tar.xz'));
                        ...
                        const nodePath = path.join(parentDir, nodeDir);
                        
                        // 安装到系统目录
                        const installDir = platform === 'darwin' ? '/usr/local' : '/usr/local';
                        
                        progress.report({ message: "安装到系统目录..." });
                        
                        // 复制文件（需要sudo权限）
                        const copyCommand = `sudo cp -R "${nodePath}"/* "${installDir}/"`;
                        
                        exec(copyCommand, (copyError: any) => {
                            if (copyError) {
                                this.outputChannel.appendLine(`❌ 安装失败: ${copyError.message}`);
                                this.outputChannel.appendLine('提示: Unix系统安装需要管理员权限');
                                resolve(false);
                                return;
                            }
            ...
            this.outputChannel.appendLine(`❌ Unix安装失败: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }

    /**
     * @description 将目录添加到Windows PATH环境变量
     */
    private async addToWindowsPath(installDir: string): Promise<boolean> {
        try {
            const { exec } = require('child_process');
            
            // 使用PowerShell添加到用户PATH
            const addPathCommand = `powershell -command "[Environment]::SetEnvironmentVariable('Path', [Environment]::GetEnvironmentVariable('Path', 'User') + ';${installDir}', 'User')"`;
            
            return new Promise((resolve) => {
                exec(addPathCommand, (error: any, stdout: string, stderr: string) => {
                    if (error) {
                        this.outputChannel.appendLine(`⚠️ 环境变量设置可能失败: ${error.message}`);
                        resolve(false);
                    } else {
                        this.outputChannel.appendLine('✅ 环境变量设置成功');
                        resolve(true);
                    }
                });
            });
            ...
            this.outputChannel.appendLine(`❌ 环境变量设置失败: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
        ...
        return new Promise<boolean>((resolve) => {
            ...
            exec(fullCommand, execOptions, (error: any, stdout: string, stderr: string) => {
                if (!error && stdout) {
                    ...
                    this.outputChannel.appendLine(`  ${command} 不可用: ${error?.message || stderr || '未知错误'}`);
                    resolve(false);
                }
            });
        });
