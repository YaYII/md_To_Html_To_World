...
class HtmlToExcelConverter {
     ...
     * 递归处理HTML元素
     */
    async processElement($, element, worksheet, parentLevel = 0, depth = 0) {
        const MAX_DEPTH = 50; // 最大递归深度
        
        if (depth > MAX_DEPTH) {
            console.warn(`HTML元素递归深度超过限制 (${MAX_DEPTH})，停止处理以防止堆栈溢出`);
            return;
        }
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            await this.processElementByType($, child, worksheet, tagName, parentLevel, depth + 1);
        }
    }
     ...
     * 根据元素类型处理
     */
    async processElementByType($, element, worksheet, tagName, parentLevel, depth = 0) {
        switch (tagName.toLowerCase()) {
            ...
            case 'div':
                // 递归处理div内容
                await this.processElement($, element, worksheet, parentLevel, depth);
                break;
    ...
    async processHeading($, element, worksheet, tagName) {
        ...
        if (text) {
            // addContentRow 已经处理了合并和智能布局，不需要重复处理
    ...
    async processParagraph($, element, worksheet) {
        ...
        if (text) {
            // addContentRow已经处理了智能布局和合并单元格，不需要重复处理
     ...
     * @param {number} totalColumns - 合并的列数
     * @param {Object} worksheet - 工作表对象
     * @returns {Object} { wrapText: boolean, rowHeight: number, estimatedLines: number, processedContent: string }
     */
    calculateMergedCellLayout(content, totalColumns, worksheet) {
        if (!content || content.length === 0) {
            return { wrapText: false, rowHeight: 25, estimatedLines: 1, processedContent: content };
        }
        
        // 计算合并后的总宽度（基于所有合并列的宽度）
        let totalWidth = 0;
        for (let i = 1; i <= totalColumns; i++) {
            const column = worksheet.getColumn(i);
            // 如果列宽没有设置，使用默认宽度16
            const columnWidth = column.width || 16;
            totalWidth += columnWidth;
        }
        
        console.log(`合并单元格布局计算: 内容长度=${content.length}, 合并列数=${totalColumns}, 总宽度=${totalWidth.toFixed(1)}`);
        
        // 计算中英文字符
        const chineseCharCount = (content.match(/[\u4e00-\u9fa5]/g) || []).length;
        const englishCharCount = content.length - chineseCharCount;
        
        // 每个字符的平均宽度（中文字符约2个单位，英文字符约1个单位）
        const totalCharWidth = chineseCharCount * 2 + englishCharCount * 1;
        
        // 根据总宽度计算一行能容纳的字符数
        // 考虑到单元格边距，实际可用宽度约为总宽度的85%
        const usableWidth = totalWidth * 0.85;
        const charsPerLine = Math.floor(usableWidth / 1.2); // 1.2是字符宽度系数
        
        console.log(`字符宽度计算: 中文=${chineseCharCount}, 英文=${englishCharCount}, 总字符宽度=${totalCharWidth}, 可用宽度=${usableWidth.toFixed(1)}, 每行字符数=${charsPerLine}`);
        
        // 判断是否需要换行
        const needsWrapping = totalCharWidth > charsPerLine || content.includes('\n');
        
        let estimatedLines = 1;
        let processedContent = content;
        
        if (needsWrapping && charsPerLine > 10) { // 确保每行至少能容纳10个字符
            if (content.includes('\n')) {
                // 如果内容本身包含换行符，处理每一行
                ...
                for (const line of lines) {
                    const wrappedLines = this.wrapTextToLines(line, charsPerLine);
                    newLines.push(...wrappedLines);
                    estimatedLines += wrappedLines.length;
                }
                
                processedContent = newLines.join('\n');
            } else {
                // 自动在适当位置插入换行符
                const wrappedLines = this.wrapTextToLines(content, charsPerLine);
        ...
        // 限制最大行数，避免行高过大
        estimatedLines = Math.min(estimatedLines, 10);
        
        // 计算行高：每行约25像素，最小25，最大250
        const calculatedHeight = Math.max(25, Math.min(250, estimatedLines * 25));
        
        console.log(`布局结果: 需要换行=${needsWrapping}, 估算行数=${estimatedLines}, 计算行高=${calculatedHeight}px, 是否添加换行=${processedContent !== content}`);
     ...
     */
    wrapTextToLines(text, charsPerLine) {
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            const charWidth = isChineseChar ? 2 : 1;
            
            // 检查加入这个字符后是否超出一行宽度
            if (currentWidth + charWidth > charsPerLine && currentLine.length > 0) {
                // 如果是英文，尝试在单词边界换行
                if (!isChineseChar && currentLine.length > 0) {
                    ...
                    if (lastSpaceIndex > 0 && (currentLine.length - lastSpaceIndex) < 15) {
                        ...
                        currentWidth = this.calculateTextWidth(currentLine);
                        continue;
                    }
                }
    ...
    calculateTextWidth(text) {
        ...
        return chineseCharCount * 2 + englishCharCount * 1;
    }
        ...
        if (isHeading && totalColumns > 1 && !isTableContent) {
            ...
            if (endCol >= startCol && endCol <= 16384) {
                try {
                    ...
                    // 使用智能布局计算来处理合并单元格的换行和行高
                    const layout = this.calculateMergedCellLayout(content, endCol - startCol + 1, worksheet);
                    const mergedCell = row.getCell(startCol);
                    
                    // 使用处理后的内容（包含换行符）
                    mergedCell.value = layout.processedContent;
                    
                    mergedCell.alignment = {
                        horizontal: 'left',
                        vertical: layout.wrapText ? 'top' : 'middle',
                        wrapText: layout.wrapText
                    };
                    
                    // 根据计算结果设置行高
                    row.height = layout.rowHeight;
                    console.log(`标题行布局: 换行=${layout.wrapText}, 行高=${layout.rowHeight}px, 估算行数=${layout.estimatedLines}, 添加换行=${layout.processedContent !== content}`)
                } catch (error) {
                    ...
                    // 如果合并失败，至少设置第一列的样式，并使用智能布局计算
                    const layout = this.calculateMergedCellLayout(content, 1, worksheet); // 回退到单列
                    const cell = row.getCell(startCol);
                    
                    // 使用处理后的内容（包含换行符）
                    cell.value = layout.processedContent;
                    ...
                    // 根据计算结果设置行高
                    row.height = layout.rowHeight;
                    console.log(`标题行回退布局: 换行=${layout.wrapText}, 行高=${layout.rowHeight}px, 添加换行=${layout.processedContent !== content}`)
                }
            }
            ...
            if (type !== 'table' && type !== 'table-reference' && totalColumns > 1 && !isTableContent) {
                ...
                try {
                    ...
                    // 根据计算结果设置行高
                    row.height = layout.rowHeight;
                    console.log(`非标题行布局: 换行=${layout.wrapText}, 行高=${layout.rowHeight}px, 估算行数=${layout.estimatedLines}, 添加换行=${layout.processedContent !== content}`)
    ...
    addContentRowFallback(worksheet, row, type, level, content) {
        ...
        if (this.config.contentMapping.includeType) {
            ...
            typeCell.value = type;
            // Type单元格通常是短标识符，一般不需要换行，但如果内容特别长也可以换行
            const layout = this.calculateMergedCellLayout(type || '', 1, worksheet);
            typeCell.alignment = {
                horizontal: 'left',
                vertical: layout.wrapText ? 'top' : 'middle',
                wrapText: layout.wrapText
            ...
            if (layout.wrapText) {
                typeCell.value = layout.processedContent;
                row.height = Math.max(row.height || 25, layout.rowHeight);
            }
            colIndex++;
        }
        ...
        if (this.config.contentMapping.includeLevel) {
            ...
            // Level单元格通常是数字或短字符串，一般不需要换行，但如果内容特别长也可以换行
            const layout = this.calculateMergedCellLayout(levelValue.toString(), 1, worksheet);
            levelCell.alignment = {
                horizontal: 'left',
                vertical: layout.wrapText ? 'top' : 'middle',
                wrapText: layout.wrapText
            ...
            if (layout.wrapText) {
                levelCell.value = layout.processedContent;
                row.height = Math.max(row.height || 25, layout.rowHeight);
            }
            colIndex++;
        }
        ...
        if (this.config.contentMapping.includeContent) {
            ...
            // 使用智能布局计算来处理内容单元格的换行和行高
            const layout = this.calculateMergedCellLayout(content, 1, worksheet); // 单列内容
            
            // 使用处理后的内容（包含换行符）
            contentCell.value = layout.processedContent;
            ...
            // 根据计算结果设置行高
            row.height = Math.max(row.height || 25, layout.rowHeight);
            console.log(`内容单元格布局: 换行=${layout.wrapText}, 行高=${layout.rowHeight}px, 添加换行=${layout.processedContent !== content}`);
    ...
    applyStyles(worksheet) {
        ...
        worksheet.eachRow((row, rowNumber) => {
            ...
            row.eachCell((cell) => {
                ...
                // 如果单元格没有设置换行属性，让Excel自动决定
                // 不再强制设置为false，让之前的智能布局决策生效
            });
        });
    }
