...
class CodeProcessor {
  ...
  process($el, $) {
    ...
    // 获取基本配置（配置已由 defaultConfig 保证非空）
    ...
    if (language) {
      ...
      const langHeightPx = Math.round(this.converter.estimateParagraphHeight ?
        this.converter.estimateParagraphHeight(language, codeSize) : 24);
      this.converter.addDocElement(langParagraph, langHeightPx);
    }
    ...
    const containerParagraph = new Paragraph({
      ...
      spacing: {
        ...
        after: 0,
        line: paragraphConfig.line_spacing * 240,
      },
      shading: {
    ...
    // 添加容器段落（估算约1行高度）
    const containerHeightPx = Math.round(codeSize * 1.2 * 96 / 72);
    this.converter.addDocElement(containerParagraph, containerHeightPx);
    ...
    if (showLineNumbers) {
      ...
      this.converter.addDocElement(codeTable, codeLines.length * containerHeightPx);
    } else {
      // 逐行处理代码，每行创建一个段落
    ...
    // 添加代码块结束边框（估算约1行高度）
    const endHeightPx = Math.round(codeSize * 1.2 * 96 / 72);
    ...
    this.converter.addDocElement(endParagraph, endHeightPx);
  }
  ...
  processCodeLines(codeLines, codeFont, codeSize, codeColor, backgroundColor) {
    ...
    for (let i = 0; i < codeLines.length; i++) {
      ...
      const codeLineHeightPx = Math.round(codeSize * 1.2 * 96 / 72);
      this.converter.addDocElement(codeParagraph, codeLineHeightPx);
    }
  }
