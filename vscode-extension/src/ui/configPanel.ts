...
export class ConfigPanel {
    ...
    private _loadDefaultConfig() {
        try {
            ...
            // 如果没有用户配置，则加载示例配置文件
            const configPath = path.join(this._extensionPath, 'src', 'mdtoworld', 'config_example.yaml');
            ...
            if (fs.existsSync(configPath)) {
                ...
                this._defaultConfig = {
                    fonts: COMPLETE_DEFAULTS.fonts,
                    sizes: COMPLETE_DEFAULTS.sizes,
                    colors: COMPLETE_DEFAULTS.colors
                };
            }
        } catch (error) {
    ...
    private _saveUserConfig(config: IDocumentConfig): void {
        try {
            ...
            notifyError(`保存配置失败: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
                ...
                <div class="tab-content" id="document">
                    ...
                    <div class="form-group">
                        ...
                        <select id="document.page_size">
                            <option value="A3" ${(config.document?.page_size ?? COMPLETE_DEFAULTS.document.page_size) === 'A3' ? 'selected' : ''}>A3</option>
                            <option value="A4" ${(config.document?.page_size ?? COMPLETE_DEFAULTS.document.page_size) === 'A4' ? 'selected' : ''}>A4</option>
                            <option value="Letter" ${(config.document?.page_size ?? COMPLETE_DEFAULTS.document.page_size) === 'Letter' ? 'selected' : ''}>Letter</option>
                            <option value="Legal" ${(config.document?.page_size ?? COMPLETE_DEFAULTS.document.page_size) === 'Legal' ? 'selected' : ''}>Legal</option>
                        </select>
                    </div>
