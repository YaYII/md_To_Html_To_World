...
// 导入 Node.js 模块
// 注意：这里使用相对路径导入，假设mdtoworld目录位于src目录下
// @ts-ignore
import NodeJsConverter from '../mdtoworld/converter';
// @ts-ignore
import ConfigManager from '../mdtoworld/utils/configManager';
// @ts-ignore
import ConfigUI from '../mdtoworld/utils/configUI';
// 单一权威配置源：所有默认值统一来自 mdtoworld/utils/defaultConfig.js
import { COMPLETE_DEFAULTS } from '../mdtoworld/utils/defaultConfig';
...
export interface IConversionOptions {
    ...
    useLegacyHtml?: boolean; // true 时使用旧版 HTML 模板（默认走政府公文规范 gov 模板）
...
interface INodeJsConverter {
    ...
    convertGovFile(inputPath: string, outputPath: string, options?: Record<string, unknown>): Promise<string>;
    md_to_html: {
...
export class NodeMarkdownConverter {
    ...
    async convert(inputFile: string, options: IConversionOptions = {}): Promise<IConversionResult> {
        try {
            ...
            const result = await this.nodeConverter.convert_file(
                inputFile, 
                outputFile, 
                options.keepHtml !== false // 默认同时产出 gov 样式 HTML（与 Word 同目录）
            );
            
            if (result.success) {
                const htmlHint = result.htmlFile ? `（另已生成 HTML: ${path.basename(result.htmlFile)}）` : '';
                const message = `成功将 ${inputBaseName}.md 转换为 ${path.basename(outputFile)}${htmlHint}`;
                console.log(message);
    ...
    async batchConvert(inputDir: string, outputDir: string, options: IConversionOptions = {}): Promise<Record<string, boolean>> {
        try {
            ...
            const results = await this.nodeConverter.batch_convert(
                inputDir, 
                outputDir, 
                options.keepHtml !== false // 默认同时产出 gov 样式 HTML
            );
    ...
    async convertToHtml(inputFile: string, options: IConversionOptions = {}): Promise<IConversionResult> {
        try {
            ...
            // 执行转换：Node 服务端渲染完成（gov 公文样式 + marked 渲染 + mermaid/图片放大增强）
            // 注意：必须走 convertString（服务端已渲染静态 DOM），而非 convertGovFile（浏览器 JS 渲染）——
            // 否则生成的 HTML 打开时依赖浏览器执行 marked.parse 才能显示，禁用 JS 即空白/原文
            if (!this.nodeConverter) {
                throw new Error('Node.js转换器未初始化');
            }
            const useLegacy = options.useLegacyHtml === true;
            const htmlContent = useLegacy
                ? await this.nodeConverter.md_to_html.convertFile(inputFile, outputFile)
                : await this.nodeConverter.md_to_html.convertFile(inputFile, outputFile);
        ...
        try {
            ...
            for (const file of files) {
                try {
                    ...
                    // 批量 HTML 转换与单个命令保持一致：默认政府公文规范 gov 模板
                    const useLegacy = options.useLegacyHtml === true;
                    const htmlContent = useLegacy
                        ? await this.nodeConverter.md_to_html.convertFile(file, outputFile)
                        : await this.nodeConverter.md_to_html.convertGovFile(file, outputFile);
    ...
    async editConfig(): Promise<void> {
        try {
            ...
            notifyError(`编辑配置失败: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    private getNodeConfigFromVSCode(vsConfig: vscode.WorkspaceConfiguration): Record<string, unknown> {
        ...
        return {
            fonts: {
                default: vsConfig.get('defaultFontFamily') ?? COMPLETE_DEFAULTS.fonts.default,
                code: COMPLETE_DEFAULTS.fonts.code,
                headings: vsConfig.get('defaultFontFamily') ?? COMPLETE_DEFAULTS.fonts.headings
            },
            sizes: {
                default: vsConfig.get('defaultFontSize') ?? COMPLETE_DEFAULTS.sizes.default,
                code: Number(vsConfig.get('defaultFontSize') ?? COMPLETE_DEFAULTS.sizes.default) - 2,
                heading1: COMPLETE_DEFAULTS.sizes.heading1,
                heading2: COMPLETE_DEFAULTS.sizes.heading2,
                heading3: COMPLETE_DEFAULTS.sizes.heading3,
                heading4: COMPLETE_DEFAULTS.sizes.heading4,
                heading5: COMPLETE_DEFAULTS.sizes.heading5,
                heading6: COMPLETE_DEFAULTS.sizes.heading6
            },
            colors: {
                default: COMPLETE_DEFAULTS.colors.default,
                headings: COMPLETE_DEFAULTS.colors.headings,
                code: COMPLETE_DEFAULTS.colors.code,
                link: COMPLETE_DEFAULTS.colors.link
            },
            paragraph: {
                line_spacing: vsConfig.get('defaultLineSpacing') ?? COMPLETE_DEFAULTS.paragraph.line_spacing,
                space_before: COMPLETE_DEFAULTS.paragraph.space_before,
                space_after: COMPLETE_DEFAULTS.paragraph.space_after,
                first_line_indent: COMPLETE_DEFAULTS.paragraph.first_line_indent
            },
            document: {
                page_size: vsConfig.get('defaultPageSize') ?? COMPLETE_DEFAULTS.document.page_size,
                orientation: vsConfig.get('defaultOrientation') ?? COMPLETE_DEFAULTS.document.orientation,
                margin_top: COMPLETE_DEFAULTS.document.margin_top,
                margin_bottom: COMPLETE_DEFAULTS.document.margin_bottom,
                margin_left: COMPLETE_DEFAULTS.document.margin_left,
                margin_right: COMPLETE_DEFAULTS.document.margin_right,
                generate_toc: vsConfig.get('includeToc') ?? COMPLETE_DEFAULTS.document.generate_toc,
                show_horizontal_rules: vsConfig.get('showHorizontalRules') ?? COMPLETE_DEFAULTS.document.show_horizontal_rules,
                toc_depth: vsConfig.get('tocDepth') ?? COMPLETE_DEFAULTS.document.toc_depth
            },
            images: {
                preserve: vsConfig.get('preserveImages') ?? COMPLETE_DEFAULTS.images.preserve,
                max_width: vsConfig.get('imageMaxWidth') ?? COMPLETE_DEFAULTS.images.max_width
            },
            output: {
                keepHtml: vsConfig.get('keepHtml') ?? COMPLETE_DEFAULTS.output.keepHtml
            },
            charts: COMPLETE_DEFAULTS.charts
        };
    }
    ...
    private convertConfig(config: IDocumentConfig): Record<string, unknown> {
        ...
        const convertedConfig: Record<string, unknown> = {
            fonts: {
                default: config.fonts?.default ?? COMPLETE_DEFAULTS.fonts.default,
                code: config.fonts?.code ?? COMPLETE_DEFAULTS.fonts.code,
                headings: config.fonts?.headings ?? COMPLETE_DEFAULTS.fonts.headings
            },
            sizes: {
                default: config.sizes?.default ?? COMPLETE_DEFAULTS.sizes.default,
                code: config.sizes?.code ?? COMPLETE_DEFAULTS.sizes.code,
                heading1: config.sizes?.heading1 ?? COMPLETE_DEFAULTS.sizes.heading1,
                heading2: config.sizes?.heading2 ?? COMPLETE_DEFAULTS.sizes.heading2,
                heading3: config.sizes?.heading3 ?? COMPLETE_DEFAULTS.sizes.heading3,
                heading4: config.sizes?.heading4 ?? COMPLETE_DEFAULTS.sizes.heading4,
                heading5: config.sizes?.heading5 ?? COMPLETE_DEFAULTS.sizes.heading5,
                heading6: config.sizes?.heading6 ?? COMPLETE_DEFAULTS.sizes.heading6
            },
            colors: {
                default: config.colors?.default ?? COMPLETE_DEFAULTS.colors.default,
                headings: config.colors?.headings ?? COMPLETE_DEFAULTS.colors.headings,
                code: config.colors?.code ?? COMPLETE_DEFAULTS.colors.code,
                link: config.colors?.link ?? COMPLETE_DEFAULTS.colors.link
            },
            paragraph: {
                line_spacing: config.paragraph?.line_spacing ?? COMPLETE_DEFAULTS.paragraph.line_spacing,
                space_before: config.paragraph?.space_before ?? COMPLETE_DEFAULTS.paragraph.space_before,
                space_after: config.paragraph?.space_after ?? COMPLETE_DEFAULTS.paragraph.space_after,
                first_line_indent: config.paragraph?.first_line_indent ?? COMPLETE_DEFAULTS.paragraph.first_line_indent
            },
            document: {
                page_size: config.document?.page_size ?? COMPLETE_DEFAULTS.document.page_size,
                orientation: config.document?.orientation ?? COMPLETE_DEFAULTS.document.orientation,
                margin_top: config.document?.margin_top ?? COMPLETE_DEFAULTS.document.margin_top,
                margin_bottom: config.document?.margin_bottom ?? COMPLETE_DEFAULTS.document.margin_bottom,
                margin_left: config.document?.margin_left ?? COMPLETE_DEFAULTS.document.margin_left,
                margin_right: config.document?.margin_right ?? COMPLETE_DEFAULTS.document.margin_right,
                generate_toc: config.document?.generate_toc ?? COMPLETE_DEFAULTS.document.generate_toc,
                show_horizontal_rules: config.document?.show_horizontal_rules ?? COMPLETE_DEFAULTS.document.show_horizontal_rules,
                toc_depth: config.document?.toc_depth ?? COMPLETE_DEFAULTS.document.toc_depth
            },
            images: {
                preserve: config.images?.preserve ?? COMPLETE_DEFAULTS.images.preserve,
                max_width: config.images?.max_width ?? COMPLETE_DEFAULTS.images.max_width
            },
            chinese: {
                // 确保繁体转换配置正确传递
                convert_to_traditional: config.chinese?.convert_to_traditional ?? COMPLETE_DEFAULTS.chinese.convert_to_traditional,
                punctuation_spacing: config.chinese?.punctuation_spacing ?? COMPLETE_DEFAULTS.chinese.punctuation_spacing,
                auto_spacing: config.chinese?.auto_spacing ?? COMPLETE_DEFAULTS.chinese.auto_spacing
            },
            charts: COMPLETE_DEFAULTS.charts
        };
