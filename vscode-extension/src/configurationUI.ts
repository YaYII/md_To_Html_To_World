...
export class ConfigurationUI {
    ...
    public async showConfigurationUI(): Promise<IDocumentConversionConfig | undefined> {
        ...
        try {
            ...
            notifyError(`配置过程出错: ${error instanceof Error ? error.message : String(error)}`);
        }
    ...
    private getDefaultConfig(inputFileUri: vscode.Uri): IDocumentConversionConfig {
        ...
        return {
            ...
            formatOptions: {
                pageSize: COMPLETE_DEFAULTS.document.page_size as 'A3' | 'A4' | 'Letter' | 'Legal',
                orientation: COMPLETE_DEFAULTS.document.orientation as 'portrait' | 'landscape',
                margins: {
                    top: COMPLETE_DEFAULTS.document.margin_top,
                    bottom: COMPLETE_DEFAULTS.document.margin_bottom,
                    left: COMPLETE_DEFAULTS.document.margin_left,
                    right: COMPLETE_DEFAULTS.document.margin_right
                }
            },
            styleOptions: {
                fontFamily: COMPLETE_DEFAULTS.fonts.default,
                fontSize: COMPLETE_DEFAULTS.sizes.default,
                lineSpacing: COMPLETE_DEFAULTS.paragraph.line_spacing,
                enableCustomStyles: false
            },
            tocOptions: {
                includeToc: COMPLETE_DEFAULTS.document.generate_toc,
                tocDepth: COMPLETE_DEFAULTS.document.toc_depth,
                tocTitle: '目录'
            },
            advancedOptions: {
                preserveImages: COMPLETE_DEFAULTS.images.preserve,
                imageMaxWidth: COMPLETE_DEFAULTS.images.max_width,
                preserveLinks: true,
